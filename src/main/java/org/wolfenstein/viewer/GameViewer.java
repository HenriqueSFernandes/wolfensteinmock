    ...
    protected void drawElements(GUI gui) throws IOException {
        ...
        for (int i = 0; i < getModel().getGuardList().size(); i++) {
            if (getModel().getGuardList().get(i).getHealth() > 0 &&
                    !getModel().getPlayer().getPosition().checkWall(getModel().getGuardList().get(i).getPosition(), getModel().getMap()) &&
                    -Position.FOV / 2.0 <= Player.getInstance().getPosition().viewAngle(getModel().getGuardList().get(i).getPosition())
                    && Player.getInstance().getPosition().viewAngle(getModel().getGuardList().get(i).getPosition()) <= Position.FOV / 2.0)
